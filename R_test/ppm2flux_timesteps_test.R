...
### 2.1.2. Loop section 2: Rate correction ####
    ...
    flux_df[[flux_var_Alt1]][i] <- coef(lm_Alt1i)[2]*60 # Returns CH4_flux_mgm2h for each Code.
    flux_df[[r2_var_Alt1]][i] <- summary(lm_Alt1i)$r.squared # Returns R2_CH4 for each Code.
    flux_df[[p_var_Alt1]][i] <- lmp_i(lm_Alt1i) # Returns p-value for each Code.

    #### Alt_2: excluding concentration from time step T1
    Filt_Alt2i <- if(is.na(Filt_i[[mass_var]][2]) == TRUE) {Filt_i} else {Filt_i[-2,]} # Filters excluding one concentration. In case the value to be
    ...
    flux_df[[flux_var_Alt2]][i] <- coef(lm_Alt2i)[2]*60 # Returns CH4_flux_mgm2h for each Code.
    flux_df[[r2_var_Alt2]][i] <- summary(lm_Alt2i)$r.squared # Returns R2_CH4 for each Code.
    flux_df[[p_var_Alt2]][i] <- lmp_i(lm_Alt2i) # Returns p-value for each Code.

    #### Alt_3: excluding concentration from time step T2
    Filt_Alt3i <- if(is.na(Filt_i[[mass_var]][3]) == TRUE) {Filt_i} else {Filt_i[-3,]} # Filters excluding one concentration. In case the value to be
    # excluded is NA it doesn't exclude values
    lm_Alt3i <- lm(as.formula(paste0(mass_var, "~Time_mins")), data=Filt_Alt3i) # Linear model of these 3 values
    flux_df[[flux_var_Alt3]][i] <- coef(lm_Alt3i)[2]*60 # Returns CH4_flux_mgm2h for each Code.
    flux_df[[r2_var_Alt3]][i] <- summary(lm_Alt3i)$r.squared # Returns R2_CH4 for each Code.
    flux_df[[p_var_Alt3]][i] <- lmp_i(lm_Alt3i) # Returns p-value for each Code.

    #### Alt_4: excluding concentration from time step T3
    Filt_Alt4i <- if(is.na(Filt_i[[mass_var]][4]) == TRUE) {Filt_i} else {Filt_i[-4,]} # Filters excluding one concentration. In case the value to be
    # excluded is NA it doesn't exclude values
    lm_Alt4i <- lm(as.formula(paste0(mass_var, "~Time_mins")), data=Filt_Alt4i) # Linear model of these 3 values
    flux_df[[flux_var_Alt4]][i] <- coef(lm_Alt4i)[2]*60 # Returns CH4_flux_mgm2h for each Code.
    flux_df[[r2_var_Alt4]][i] <- summary(lm_Alt4i)$r.squared # Returns R2_CH4 for each Code.
    ...
    flux_df$CH4_flux_corrected[i] <- if((flux_df[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {0
              ...
              (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) & (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i])) {flux_df[[mass_va]][i]
    } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) & (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {flux_df[[flux_var_Alt1]][i]
    } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {flux_df[[flux_var_Alt2]][i]
    } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {flux_df[[flux_var_Alt3]][i]
    } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {flux_df[[flux_var_Alt4]][i]
    } else {0}
    ...
    flux_df$CH4_model[i] <- if((flux_df[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {"No flux"
              ...
              (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) & (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i])){"Original"
    } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) & (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {"Alt. 1"
    } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {"Alt. 2"
    } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {"Alt. 3"
    } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {"Alt. 4"
    } else {"No flux"}
    ...
    flux_df$R2_CH4_corrected[i] <- if((flux_df[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {0
              ...
              (flux_df[[r2_var_Alt3]][i] < 0.7) & (flux_df[[r2_var_Alt4]][i] < 0.7)) {0
               ...
               (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {flux_df[[r2_var_Alt1]][i]
    } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {flux_df[[r2_var_Alt2]][i]
    } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {flux_df[[r2_var_Alt3]][i]
    } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {flux_df[[r2_var_Alt4]][i]
    } else {0}
    #
    ...
    flux_df$Logic_CH4[i] <- if((flux_df[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {"Original model has negative rate and R2 < 0.7"
              ...
              (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) & (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i]))  {"Original model achieves the highest R2"
    } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) & (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {"Original model has R2 threshold and Alt. 1 achieves the highest
                  R2 (> 0.7) w/positive rate"
    } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
               (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {"Original model has R2 threshold and Alt. 2 achieves the highest
                  R2 (> 0.7) w/positive rate"
    } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {"Original model has R2 threshold and Alt. 3 achieves the highest
                  R2 (> 0.7) w/positive rate"
    } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) & (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
               (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {"Original model has R2 threshold and Alt. 4 achieves the highest
                  R2 (> 0.7) w/positive rate"
