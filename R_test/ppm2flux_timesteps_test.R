...
ppm2flux_test <- function(data, Timesteps = 4, CH4_mass = 0, N2O_mass = 0, CO2_mass = 0, Gas1_mass = 0,  Gas2_mass = 0, Gas3_mass = 0, Diagnostics = FALSE) {
  ...
  ##
  ## CH4_mass/N2O_mass/CO2_mass/Gas1_mass/Gas2_mass/Gas3_mass -> Molar mass of Gas molecule (or target atom - e.g. C in CH4-C - depending on GC settings)
  ##              Default: 0. e.g. If the user wants to run the function for CH4, N2O and two additional gases then:
  ##              CH4_mass = 16, N2O_mass = 44,  Gas1_mass = molar mass of Gas1,  Gas2_mass = molar mass of Gas1.
  ##
  ## Diagnostics -> Allows the user to decide if diagnostic plots are created or not. Default = FALSE (not creating them)
  ...
  if (Timesteps == 4) { # in case Timesteps argument is left as default
    ...
    colnames(flux_df_test) = c("Code_Nr", "ID", "Date", "CH4_flux_mgm2h", "R2_CH4",
  ...
  for(gas in Gases) {
    ...
    for (i in 1:length(flux_df_test$ID)) {
      ...
      if (Timesteps == 4 & gas_mass != 0) { # in case Timesteps argument is left as default then alternative models are calculated
        ...
        flux_df_test[[paste0(gas, "_flux_corrected")]][i] <- if((flux_df_test[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {0
        } else if(flux_df_test[[r2_var]][i] > 0.7) {flux_df_test[[flux_var]][i]
        } else if((flux_df_test[[r2_var]][i] < 0.7) & (flux_df_test[[r2_var_Alt1]][i] < 0.7) & (flux_df_test[[r2_var_Alt2]][i] < 0.7) &
                  (flux_df_test[[r2_var_Alt3]][i] < 0.7) & (flux_df_test[[r2_var_Alt4]][i] < 0.7)) {0
        } else if((flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                  (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt3]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt4]][i])) {flux_df_test[[mass_va]][i]
        } else if ((flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt2]][i]) & (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {flux_df_test[[flux_var_Alt1]][i]
        } else if ((flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {flux_df_test[[flux_var_Alt2]][i]
        } else if ((flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {flux_df_test[[flux_var_Alt3]][i]
        } else if ((flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {flux_df_test[[flux_var_Alt4]][i]
        } else {0}

        # ## Column with chosen model:
        flux_df_test[[paste0(gas, "_model")]][i] <- if((flux_df_test[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {"No flux"
        } else if(flux_df_test[[r2_var]][i] > 0.7) {"Original"
        } else if((flux_df_test[[r2_var]][i] < 0.7) & (flux_df_test[[r2_var_Alt1]][i] < 0.7) & (flux_df_test[[r2_var_Alt2]][i] < 0.7) &
                  (flux_df_test[[r2_var_Alt3]][i] < 0.7) & (flux_df_test[[r2_var_Alt4]][i] < 0.7)) {"No flux"
        } else if((flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                  (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt3]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt4]][i])){"Original"
        } else if ((flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt2]][i]) & (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {"Alt. 1"
        } else if ((flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {"Alt. 2"
        } else if ((flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {"Alt. 3"
        } else if ((flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {"Alt. 4"
        } else {"No flux"}

        # ## Loop section 2.3: Calculating R2 according to the applied correction (if any):
        flux_df_test[[paste0("R2_", gas, "_corrected")]][i] <- if((flux_df_test[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {0
        } else if(flux_df_test[[r2_var]][i] > 0.7) {flux_df_test[[r2_var]][i]
        } else if((flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                  (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt3]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt4]][i])) {flux_df_test[[r2_var]][i]
        } else if((flux_df_test[[r2_var]][i] < 0.7) & (flux_df_test[[r2_var_Alt1]][i] < 0.7) & (flux_df_test[[r2_var_Alt2]][i] < 0.7) &
                  (flux_df_test[[r2_var_Alt3]][i] < 0.7) & (flux_df_test[[r2_var_Alt4]][i] < 0.7)) {0
        } else if ((flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt2]][i]) & (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {flux_df_test[[r2_var_Alt1]][i]
        } else if ((flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {flux_df_test[[r2_var_Alt2]][i]
        } else if ((flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {flux_df_test[[r2_var_Alt3]][i]
        } else if ((flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {flux_df_test[[r2_var_Alt4]][i]
        } else {0}
        #
        # ## Add column with method and rate selection logic:
        flux_df_test[[paste0("Logic_", gas)]][i] <- if((flux_df_test[[r2_var]][i] < 0.7) & coef(lm_i)[2] < 0) {"Original model has negative rate and R2 < 0.7"
        } else if(flux_df_test[[r2_var]][i] > 0.7) {"Original model has R2 > 0.7"
        } else if((flux_df_test[[r2_var]][i] < 0.7) & (flux_df_test[[r2_var_Alt1]][i] < 0.7) & (flux_df_test[[r2_var_Alt2]][i] < 0.7) &
                  (flux_df_test[[r2_var_Alt3]][i] < 0.7) & (flux_df_test[[r2_var_Alt4]][i] < 0.7)) {"No model achieves  R2 threshold"
        } else if((flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                  (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt3]][i]) & (flux_df_test[[r2_var]][i] > flux_df_test[[r2_var_Alt4]][i]))  {"Original model achieves the highest R2"
        } else if ((flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt2]][i]) & (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt1]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt1i)[2] > 0)) {"Original model has R2 threshold and Alt. 1 achieves the highest
                  R2 (> 0.7) w/positive rate"
        } else if ((flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt3]][i]) &
                   (flux_df_test[[r2_var_Alt2]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt2i)[2] > 0)) {"Original model has R2 threshold and Alt. 2 achieves the highest
                  R2 (> 0.7) w/positive rate"
        } else if ((flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt3]][i] > flux_df_test[[r2_var_Alt4]][i]) & (coef(lm_Alt3i)[2] > 0)) {"Original model has R2 threshold and Alt. 3 achieves the highest
                  R2 (> 0.7) w/positive rate"
        } else if ((flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt1]][i]) & (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt2]][i]) &
                   (flux_df_test[[r2_var_Alt4]][i] > flux_df_test[[r2_var_Alt3]][i]) & (coef(lm_Alt4i)[2] > 0)) {"Original model has R2 threshold and Alt. 4 achieves the highest
                  R2 (> 0.7) w/positive rate"
