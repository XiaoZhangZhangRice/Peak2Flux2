...
ppm2flux<- function(data,
  ...
  if (Timesteps == 4) { # in case Timesteps argument is left as default
    ...
    colnames(flux_df) = c("Code_Nr", "ID", "Date", "Plot", "Treatment", "CH4_flux_mgm2h", "R2_CH4",
  ...
  for(gas in Gases) {
    ...
    for (i in 1:length(flux_df$ID)) {
      ...
      if (Timesteps == 4 & gas_mass != 0 & flux_df_test_timesteps == 4) { # in case Timesteps argument is left as default then alternative models are calculated
        ...
        flux_df[[flux_var_Alt1]][i] <- coef(lm_Alt1i)[2]*60 # Returns flux_mgm2h for each Code.
        flux_df[[r2_var_Alt1]][i] <- summary(lm_Alt1i)$r.squared # Returns R2 for each Code.
        flux_df[[p_var_Alt1]][i] <- lmp_i(lm_Alt1i) # Returns p-value for each Code.
        ...
        flux_df[[flux_var_Alt2]][i] <- coef(lm_Alt2i)[2]*60 # Returns flux_mgm2h for each Code.
        flux_df[[r2_var_Alt2]][i] <- summary(lm_Alt2i)$r.squared # Returns R2for each Code.
        flux_df[[p_var_Alt2]][i] <- lmp_i(lm_Alt2i) # Returns p-value for each Code.
        ...
        flux_df[[flux_var_Alt3]][i] <- coef(lm_Alt3i)[2]*60 # Returns _flux_mgm2h for each Code.
        flux_df[[r2_var_Alt3]][i] <- summary(lm_Alt3i)$r.squared # Returns R2 for each Code.
        flux_df[[p_var_Alt3]][i] <- lmp_i(lm_Alt3i) # Returns p-value for each Code.
        ...
        flux_df[[flux_var_Alt4]][i] <- coef(lm_Alt4i)[2]*60 # Returns flux_mgm2h for each Code.
        flux_df[[r2_var_Alt4]][i] <- summary(lm_Alt4i)$r.squared # Returns R2 for each Code.
        flux_df[[p_var_Alt4]][i] <- lmp_i(lm_Alt4i) # Returns p-value for each Code.
        ...
        flux_df[[paste0(gas, "_flux_corrected")]][i] <- if((flux_df[[r2_var]][i] > R2_Threshold) &
                                                                (Neg_Rate || coef(lm_i)[2] > 0)) {flux_df[[flux_var]][i]
        } else if((flux_df[[r2_var]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt1]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt2]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt3]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt4]][i] < R2_Threshold)) {0
        } else if((flux_df[[r2_var]][i] > flux_df[[r2_var_Alt1]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt2]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i])) {flux_df[[mass_va]][i]
        } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt1i)[2] > 0)) {flux_df[[flux_var_Alt1]][i]
        } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt2i)[2] > 0))  {flux_df[[flux_var_Alt2]][i]
        } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt3i)[2] > 0)) {flux_df[[flux_var_Alt3]][i]
        } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (Neg_Rate || coef(lm_Alt4i)[2] > 0)) {flux_df[[flux_var_Alt4]][i]
        } else {0}

        # ## Column with chosen model:
        flux_df[[paste0(gas, "_model")]][i] <- if((flux_df[[r2_var]][i] > R2_Threshold) &
                                                       (Neg_Rate || coef(lm_i)[2] > 0)) {"Complete model"
        } else if((flux_df[[r2_var]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt1]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt2]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt3]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt4]][i] < R2_Threshold)) {"No flux"
        } else if((flux_df[[r2_var]][i] > flux_df[[r2_var_Alt1]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt2]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i])){"Complete model"
        } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) &
        ...
        flux_df[[paste0("R2_", gas, "_corrected")]][i] <- if((flux_df[[r2_var]][i] > R2_Threshold) &
                                                                  (Neg_Rate || coef(lm_i)[2] > 0)) {flux_df[[r2_var]][i]
        } else if((flux_df[[r2_var]][i] > flux_df[[r2_var_Alt1]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt2]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i])) {flux_df[[r2_var]][i]
        } else if((flux_df[[r2_var]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt1]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt2]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt3]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt4]][i] < R2_Threshold)) {0
                   ...
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt1i)[2] > 0)) {flux_df[[r2_var_Alt1]][i]
        } else if ((flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt2]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt2i)[2] > 0)) {flux_df[[r2_var_Alt2]][i]
        } else if ((flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt3]][i] > flux_df[[r2_var_Alt4]][i]) &
                   (Neg_Rate || coef(lm_Alt3i)[2] > 0)) {flux_df[[r2_var_Alt3]][i]
        } else if ((flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt1]][i]) &
                   (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt4]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (Neg_Rate || coef(lm_Alt4i)[2] > 0)) {flux_df[[r2_var_Alt4]][i]
        } else {0}

        # ## Add column with method and rate selection logic:
        flux_df[[paste0("Logic_", gas)]][i] <- if((flux_df[[r2_var]][i] > R2_Threshold) &
                                                       (Neg_Rate || coef(lm_i)[2] > 0)) {"Complete model has R2 > R2_Threshold"
        } else if((flux_df[[r2_var]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt1]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt2]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt3]][i] < R2_Threshold) &
                  (flux_df[[r2_var_Alt4]][i] < R2_Threshold)) {"No model achieves  R2 threshold"
        } else if((flux_df[[r2_var]][i] > flux_df[[r2_var_Alt1]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt2]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt3]][i]) &
                  (flux_df[[r2_var]][i] > flux_df[[r2_var_Alt4]][i]))  {"Complete model achieves the highest R2"
        } else if ((flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt2]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt3]][i]) &
                   (flux_df[[r2_var_Alt1]][i] > flux_df[[r2_var_Alt4]][i]) &
        ...
        if (Diagnostics == TRUE & gas_mass != 0 & flux_df_test_timesteps == 4) { # in case Diagnostics argument changed to TRUE: creating diagnostic plots
...
# input: input_test
flux_df_testA <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44) # Test 1: Keeping all arguments as default - Output with alternative models but no diagnostics
flux_df_testB <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44, Diagnostics = TRUE) # Test 2: Activating diagnostic plots - Output with alternative models and diagnostics
flux_df_testC <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44, Timesteps = 5) # Test 3: Testing Timesteps - Output without alternative models nor diagnostics
flux_df_testD <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44, Diagnostics = TRUE, Timesteps = 5) # Test 4: No Diagnostics and Timesteps conflicts - Output without alternative models nor diagnostics
flux_df_testE <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44, Neg_Rate = FALSE) # Test 5: Testing Neg_Rate argument, does not consider alternative models resulting in negative rates.
flux_df_testF <- ppm2flux(input_test, CH4_mass = 16, N2O_mass = 44, Diagnostics = TRUE, Neg_Rate = FALSE) # Test 6: Tests Diagnostic plots with Neg_Rate modified to FALSE

# input: input_test2
flux_df_testG <- ppm2flux(input_test2, CH4_mass = 16, N2O_mass = 44) # Test 7: 2nd test data set. Keeping all arguments as default - Output with alternative models but no diagnostics
flux_df_testH <- ppm2flux(input_test2, CH4_mass = 16, N2O_mass = 44, Diagnostics = TRUE) # Test 8: Activating diagnostic plots - Output with alternative models and diagnostics
...
flux_df_testJ <- ppm2flux(input_test4, CH4_mass = 16, N2O_mass = 44, CO2_mass = 44, Timesteps = 5) # Test 9: 4th test data set (with 5 timesteps). Keeping all arguments as default - Output without alternative models nor diagnostics
flux_df_testK <- ppm2flux(input_test4, CH4_mass = 16, N2O_mass = 44, CO2_mass = 44, Diagnostics = TRUE) # Test 10: Activating diagnostic plots - Output without alternative models nor diagnostics (due to 5 timesteps within input data frame)

# input: input_test5 - 5 timesteps; 16 sampling events
flux_df_testL <- ppm2flux(input_test5, CH4_mass = 16, N2O_mass = 44, CO2_mass = 44, Timesteps = 5) # Test 11: 4th test data set (with 5 timesteps). Keeping all arguments as default - Output without alternative models nor diagnostics
flux_df_testM <- ppm2flux(input_test5, CH4_mass = 16, N2O_mass = 44, CO2_mass = 44, Diagnostics = TRUE) # Test 12: Activating diagnostic plots - Output without alternative models nor diagnostics (due to 5 timesteps within input data frame)
