...
1.  **In R_test**:
  - **ppm2flux_timesteps_test.R**:
    - **Timesteps**: by default (4) it already calculates "drop-one" alternative models and generates a diagnostics pdf. When modified, output data frame contains only original model results.
    ...
    - **Neg_Rate**: Default = TRUE.  When modified to FALSE then it includes an extra restriction to the flux correction that excludes alternative models that result in negative flux (even if resulting R<sup>2</sup> is higher than the original model). 
=======
# Peak2Flux R Package for calculating GHG fluxes from Chromatography reads.
